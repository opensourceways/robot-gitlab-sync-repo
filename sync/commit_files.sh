...
cat $all_files | xargs egrep "^oid sha256:.{64}$" | awk -F ':oid sha256:' '{print $1}' > $lfs_files
if [ ! -s $lfs_files ]; then
...
if [ $n -eq 1 ]; then
    ...
    echo_message "small" "no" "$all_files" "yes"

    exit 0
fi

small_files=$work_dir/${commit}_small
cat $all_files $lfs_files | sort | uniq -u > $small_files
